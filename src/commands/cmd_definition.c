...
bool add_function(char *name, char *left, char *right)
{
    ...
    if (redefined_rule == NULL)
    {
        ...
        if (!arith_parse_input_extended(left, FMT_ERROR_LEFT, false, false, false, &left_n))
        {
            goto error;
        }
    }
    ...
    if (!arith_parse_input_extended(right, FMT_ERROR_RIGHT, false, false, false, &right_n))
    {
        goto error;
    }
